...
# Initialize PaddleOCR (FAST version)
...
def extract_vendor_name(text: str) -> str:
    ...
    for line in lines[:5]:  # Look at the first 5 lines
        ...
        if (
            ...
            not any(c in clean_line.lower() for c in ["invoice", "date", "total", "amount", "receipt", "order"])
        ):
            normalized_name = clean_line.title().strip()  # ✅ Normalize to Title Case
    ...
    try:
        result = extract_with_gpt(
            text,
            "Extract the main vendor/store name from this invoice. Only return the name, without extra text."
        )
        return result.title().strip()  # ✅ Normalize GPT result as well
    except:
        return "Unknown"
