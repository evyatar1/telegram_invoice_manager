...
// Modified: Changed the key for chart data from 'categories_summary' to 'chart_data'
export const sendChartToTelegram = (token, categoriesSummary) =>
  API.post("/send-chart-to-telegram", { chart_data: categoriesSummary }, { // Changed to chart_data
    headers: { Authorization: `Bearer ${token}` },
  }).then(r => r.data);
