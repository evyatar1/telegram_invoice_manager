...
const generateCsvBuffer = async (invoices) => {
...
function App() {
  ...
  }, [setInvoices, calculateChartData, setMessage, clearMessage, setToken, setView]);

  useEffect(() => {
    if (token && view === "dashboard") {
      fetchUserInvoices(token);
    }
  }, [token, view, fetchUserInvoices]);
  ...
  const handleDownloadExcel = async () => {
    ...
    const buf = await generateCsvBuffer(invoices);
...
//  };
//
//const calculateChartData = useCallback((currentInvoices) => { // Removed currentExcludedCategories
//    const categories = currentInvoices.reduce((acc, invoice) => {
//      const category = invoice.category || "Uncategorized";
//      acc[category] = (acc[category] || 0) + 1;
//      return acc;
...
//      ],
//    };
//  }, []); // Dependencies for useCallback should now be empty or only include things like 'invoices' if it's not passed as a param
//
