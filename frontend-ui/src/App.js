...
//    }
//
//    const headers = ["ID", "S3 Key", "Created At", "Status", "Category", "Extracted Data"];
//    const csvRows = [];
//    csvRows.push(headers.join(","));
//
//    invoices.forEach(invoice => {
//      const extractedDataString = invoice.extracted_data ? JSON.stringify(invoice.extracted_data).replace(/"/g, '""') : '';
//      const row = [
//        invoice.id,
//        `"${invoice.s3_key}"`,
//        invoice.created_at,
...
//      document.body.removeChild(link);
//    } else {
//      setMessage("Your browser does not support downloading files directly. Please copy the data.");
//      clearMessage();
//    }
//  };
//
...
//        await deleteInvoice(invoiceToDelete, token);
//        setMessage("Invoice deleted successfully!");
//        await fetchUserInvoices(token); // Refresh invoices after deletion
//      } catch (error) {
//        setMessage(`Failed to delete invoice: ${error.response?.data?.detail || error.message}`);
//      } finally {
//        clearMessage();
...
//
//
//  // Memoize fetchUserInvoices to ensure its stability for TaskPoller's useEffect dependency
//  const fetchUserInvoices = useCallback(async (userToken) => {
//    if (userToken) {
//      try {
//        const data = await fetchInvoices(userToken);
//        setInvoices(data);
//        const categories = data.reduce((acc, invoice) => {
//          const category = invoice.category || "Uncategorized";
//          acc[category] = (acc[category] || 0) + 1;
...
//              data: Object.values(categories),
//              backgroundColor: [
//                "#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF", "#FF9F40", "#6A5ACD", "#20B2AA", "#7B68EE", "#FFD700"
//              ],
//              hoverBackgroundColor: [
//                "#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF", "#FF9F40", "#6A5ACD", "#20B2AA", "#7B68EE", "#FFD700"
//              ],
...
//      }
//    }
//  }, [setInvoices, setChartData, setMessage, clearMessage, setToken, setView]); // Dependencies for useCallback
//
//  useEffect(() => {
//    if (token && view === "dashboard") {
//      // Initial fetch when entering dashboard
//      fetchUserInvoices(token);
//    }
//  }, [token, view, fetchUserInvoices]); // Added fetchUserInvoices to dependencies
//
//
//
//
...
//      setMessage(response.data.message || "File uploaded successfully!");
//      clearMessage();
//      fetchUserInvoices(token); // Refresh invoices after upload
//      fileInputRef.current.value = null;
//    } catch (error) {
//      setMessage(`Upload failed: ${error.response?.data?.detail || error.message}`);
//      clearMessage();
...
//      setSendCsvMessage(response.message || "CSV sent to Telegram successfully!");
//      clearSendCsvMessage();
//      fetchUserInvoices(token); // Refresh invoices immediately after web-initiated send
//    } catch (error) {
//      setSendCsvMessage(`Failed to send CSV to Telegram: ${error.response?.data?.detail || error.message}`);
//      clearSendCsvMessage();
//    } finally {
...
//            <input
//              type="text"
//              placeholder="Telegram Chat ID (Optional, for OTP)"
//              className="w-full p-3 mb-6 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
//              value={telegramChatId}
//              onChange={(e) => setTelegramChatId(e.target.value)}
//            />
...
//
//
//          <TaskPoller token={token} onUpdate={fetchUserInvoices} />
//          {/* Custom Confirmation Modal */}
//          {showConfirmModal && (
//            <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
//              <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm">
...
//
//  // Custom confirmation dialog (since window.confirm is blocked in some environments)
//  const customConfirm = (messageText, onConfirmAction) => {
//    setMessage(messageText);
//    setShowConfirmModal(true);
//    // Store the action to perform on confirmation (e.g., the invoice ID for deletion)
//    setInvoiceToDelete(onConfirmAction);
...
//          </div>
//
//          //<TaskPoller token={token} onNewInvoiceProcessed={fetchUserInvoices} />
//          <TaskPoller token={token} onUpdate={fetchUserInvoices} />
//          {/* Custom Confirmation Modal */}
//          {showConfirmModal && (
//            <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
